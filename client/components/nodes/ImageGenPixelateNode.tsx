...
interface ImageGenPixelateNodeProps {
    node: Node
...
export const ImageGenPixelateNode: React.FC<ImageGenPixelateNodeProps> = ({
    node,
    connectedInputText,
    connectedInputImages = [],
    onExpand,
    onRun,
}) => {
    const { prompt, imageResources, isLoading, error, model, imageCount = 1, pixelateSize = 64, paletteEnabled = true, paletteSize = 32 } = node.data
    ...
    return (
        <div className="flex flex-col gap-3">
            {/* Model + Pixelate params row */}
            <div className="flex items-center justify-between">
                <span className="text-[10px] font-semibold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Model</span>
                <span className="flex items-center gap-1 text-[10px] text-teal-600 dark:text-teal-400 font-medium truncate max-w-[140px]" title={modelLabel}>
                    <ProviderIcon provider={modelProvider} />
                    {modelLabel}
                </span>
            </div>

            {/* Pixelate params badges */}
            <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 font-mono">
                    {pixelateSize}px
                </span>
                {paletteEnabled ? (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 font-mono">
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-zinc-800 text-slate-400 dark:text-zinc-500 font-mono">
                        no palette
                    </span>
                )}
            </div>
            ...
            {hasImages && (
                <div className="flex items-center gap-2">
                    <div className="w-10 h-10 rounded-md flex items-center justify-center border relative overflow-hidden bg-pink-100 dark:bg-pink-900/20 border-pink-300 dark:border-pink-700">
                        {inputThumbError ? (
                            <ImageOff size={14} className="text-pink-400" />
                        ) : (
                            <img
                                src={connectedInputImages[0]}
                                alt="Input"
                                className="w-full h-full object-cover rounded-md"
                                onError={() => setInputThumbError(true)}
                            />
                        )}
                        {connectedInputImages.length > 1 && (
                            <div className="absolute bottom-0 right-0 bg-pink-500 text-white text-[8px] font-bold px-1 rounded-tl-md">
                                +{connectedInputImages.length - 1}
                            </div>
                        )}
                    </div>
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-pink-500">{connectedInputImages.length} Connected</span>
                    </div>
                </div>
            )}
                    ...
                    {imageResources.map((item, idx) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div
                                    className={`absolute right-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-teal-500/50 rounded-l-full blur-[2px] transition-opacity ${imageResources.length > 1 && idx % 2 === 0 ? 'opacity-0' : 'opacity-100'}`}
                                />
                            </div>
                        )
                    })}
            ...
            <button
                ...
                className="flex items-center justify-center gap-2 flex-1 py-1.5 bg-teal-600 hover:bg-teal-500 disabled:bg-slate-300 dark:disabled:bg-zinc-700 text-white text-sm font-medium rounded-md transition-colors"
            >
